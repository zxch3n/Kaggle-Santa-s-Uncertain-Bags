(function(){
    ...
    window.panoram_partner_id = 'mtz1001';
    window.panoram_partner_key = '36108';

    if (Math.random() < 0.1) {
      var img = new Image();
      img.src = '//srv.dfgio.com/analytics/piwik.php?idsite=1&rec=1&_rcn=' + window.panoram_partner_id + '&urlref=http://' + encodeURIComponent(window.location.hostname) + '/';
    }
...
(function(window, undefined){
    ...
    var addCouponBar = function($, deals) {
        ...
        loadScript('https://srv.dfgio.com//coupons/themes/' + theme + '/theme.js', function() {
...
/*            window._gaq.push(
    ...
    if (cachedDomain && cachedDomain == window.location.hostname && cachedDeals) {
        if (cachedDeals && cachedDeals.length > 0) {
            ...
            var img = new Image();
            img.src = 'https://srv.dfgio.com//coupons/cookie.php?merchant=' + currentDomain + '&client=' + window.panoram_partner_id;
        }
    } else if (!referrer || (referrer && referrer.indexOf('afsrc=1') == -1)) {
        JSONP('https://srv.dfgio.com//coupons/deals.php?merchant=' + currentDomain + '&referrer=' + encodeURIComponent(referrer) + '&partner=' + window.panoram_partner_id, function(deals) {
